...
def resize_worker(img_files, sizes, resample, lmdb_save=False):
    lr_img = Image.open(img_files[0])
    lr_img = lr_img.convert('RGB')
    hr_img = Image.open(img_files[1])
...
def prepare_process_worker(wctx, lr_subset, hr_subset):
    for lr, hr in zip(lr_subset, hr_subset):
        # while True:
        #     try:
        i, imgs = wctx.resize_fn([lr, hr])
...
def prepare(img_path, out_path, n_worker, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    ...
    hr_files = [p for p in Path('{}'.format(img_path)).glob(f'hr/*')] #[19522:]
    ...
    if n_worker > 1:
        ...
        hr_file_subsets = np.array_split(hr_files, n_worker)
        worker_threads = []
        wctx = WorkingContext(resize_fn, lmdb_save, out_path, multi_env, sizes)

        # start worker processes, monitor results
        for i in range(n_worker):
            proc = Process(target=prepare_process_worker, args=(wctx, lr_file_subsets[i], hr_file_subsets[i]))
            proc.start()
